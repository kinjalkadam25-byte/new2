          ...
          <div class="card section-gap">
            ...
            <div class="form-group">
              ...
              <input type="text" id="expNote"
                placeholder="e.g. UPI/P2M/MUMBAI METRO ONE/AXIS BANK"
                oninput="autoFillCategory(this.value)" />
            </div>

            <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
              <div class="form-group">
                <label style="color:var(--green)">Credit (₹)</label>
                <input type="number" id="expAmtCredit" placeholder="0.00" min="0" step="0.01" />
              </div>
              <div class="form-group">
                <label style="color:var(--red)">Debit (₹)</label>
...
// 
const APPS_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbz8ApSqJ1Nn6nV_FlF_-3pRKKdYyszZ254Ah24-vLGye3tkhccIRpP-rp1chHx6Hh2w/exec";
// 
...
async function init() {
  ...
  // Populate filter category select only
...
// setType kept for compatibility
function setType(type) {}

function autoFillCategory(text) {
  // kept for compatibility — no-op since category field removed
}
...
async function addExpense() {
  ...
  const creditAmt   = parseFloat(document.getElementById('expAmtCredit').value) || 0;
  const debitAmt    = parseFloat(document.getElementById('expAmt').value) || 0;

  if (!date) { showToast('Please select a Tran Date', 'error'); return; }
  if (creditAmt <= 0 && debitAmt <= 0) { showToast('Enter a Credit or Debit amount', 'error'); return; }
  if (creditAmt > 0 && debitAmt > 0)   { showToast('Fill either Credit or Debit, not both', 'error'); return; }

  const type = creditAmt > 0 ? 'credit' : 'debit';
  const amt  = creditAmt > 0 ? creditAmt : debitAmt;
  // Auto-classify from particulars
  const cat  = autoClassifyReturn(note) || 'Other';
  ...
  try {
    ...
    document.getElementById('expAmtCredit').value = '';
...
//  Excel / CSV Parser
// Hardcoded for Axis Bank statement format:
...
function parseExcelCSV(file) {
  ...
  reader.onload = function(e) {
    try {
      ...
      console.log('File rows:', allRows.length, ' | First 3:', JSON.stringify(allRows.slice(0,3)));

      // Step 1: Find the header row
      // Your file: row 0 = blank, row 1 = ["Tran Date","PARTICULARS","Credit ","       Debit "]
      let hdrIdx = -1;
      for (let i = 0; i < Math.min(allRows.length, 10); i++) {
        const joined = allRows[i].map(v => String(v).toLowerCase().trim()).join('|');
        if (joined.includes('date') && (joined.includes('particular') || joined.includes('debit'))) {
          hdrIdx = i;
          break;
        }
      }
      if (hdrIdx === -1) { showToast('Could not find header row (Tran Date / PARTICULARS)', 'error'); return; }
      console.log('Header at row', hdrIdx, ':', allRows[hdrIdx]);

      // Step 2: Map column positions from header (trim all whitespace)
      const hdr = allRows[hdrIdx].map(v => String(v).toLowerCase().replace(/\s+/g, ''));
      let colDate = -1, colDesc = -1, colCredit = -1, colDebit = -1;
      hdr.forEach((h, i) => {
        if (h.includes('date'))                                     colDate   = i;
        if (h.includes('particular') || h.includes('narr'))        colDesc   = i;
        if (h.includes('credit') || h.includes('deposit'))         colCredit = i;
        if (h.includes('debit') || h.includes('withdraw'))         colDebit  = i;
      });

      // Fallback to positional defaults (col 0,1,2,3) if detection fails
      if (colDate   < 0) colDate   = 0;
      if (colDesc   < 0) colDesc   = 1;
      if (colCredit < 0) colCredit = 2;
      if (colDebit  < 0) colDebit  = 3;
      console.log('Cols → date:', colDate, 'desc:', colDesc, 'credit:', colCredit, 'debit:', colDebit);
      ...
      dataRows.forEach((row, i) => {
        ...
        const rawCredit = String(row[colCredit] ?? '').trim().replace(/[,\s₹]/g, '');
        const rawDebit  = String(row[colDebit]  ?? '').trim().replace(/[,\s₹]/g, '');
        ...
        if (!date) { skipped.push(rawDate || '(blank)'); return; }

        const creditAmt = parseFloat(rawCredit) || 0;
        const debitAmt  = parseFloat(rawDebit)  || 0;

        // Skip rows with no amount
        if (creditAmt <= 0 && debitAmt <= 0) { skipped.push(rawDate + ' ' + rawDesc.slice(0,20)); return; }

        const { cat, confidence } = autoClassify(rawDesc);
      ...
      console.log('Parsed:', result.length, 'rows | Skipped:', skipped.length, skipped.slice(0,5));

      if (!result.length) {
        showToast('No transactions found. Check console for details.', 'error');
        return;
      }
